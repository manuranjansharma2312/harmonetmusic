...
export default function VevoReports() {
  ...
  const fetchReports = async () => {
    ...
    if (role === 'admin' && isImpersonating && impersonatedUserId) {
      ...
      const { data } = await supabase.from('vevo_report_entries').select('*').in('isrc', ownedIsrcs).order('reporting_month', { ascending: false });
      setEntries((data as ReportEntry[]) || []); setLoading(false); return;
    }
    const { data } = await supabase.from('vevo_report_entries').select('*').order('reporting_month', { ascending: false });
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vevo_report_entries' }, () => fetchReports())
      .subscribe();
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-muted-foreground text-sm">
              {selectedMonth ? `Viewing report for ${selectedMonth}` : 'Monthly Vevo revenue reports'} · All amounts in ₹ (INR)
            </p>
          </div>
          {selectedMonth && (
          ...
          <GlassCard className="p-0 overflow-hidden">
            {monthlyGroups.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground flex flex-col items-center gap-3">
                ...
                <p>No Vevo reports available yet.</p>
              </div>
            ) : (
              <>
