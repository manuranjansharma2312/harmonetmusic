...
export default function AdminYouTubeReports() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < toInsert.length; i += 500) {
        ...
        const { error } = await supabase.from('youtube_report_entries').insert(batch);
  ...
  const handleDeleteMonth = async () => {
    ...
    const { error } = await supabase.from('youtube_report_entries').delete().eq('reporting_month', deleteMonth);
  ...
  return (
    <DashboardLayout>
      ...
      {deleteMonth && (
        <ConfirmDialog
          title="Delete YouTube Report"
          message={`Are you sure you want to delete all YouTube report entries for "${deleteMonth}"? This cannot be undone.`}
